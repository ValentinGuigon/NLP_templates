...
## Access a repository content

A quick way to access the contents of a repository on the 🤗 Hub is via the list_repo_files() function of the huggingface_hub library:

`from huggingface_hub import list_repo_files`
`list_repo_files(repo_id=model_checkpoint)`

## Use a debugger in a notebook

In a separate chunk, you can execute a cell with:
`%debug`
By executing that cell, you can type commands to debug the script:
* **u** gets you one step upwards in the traceback
* **d** gets you one step downwards in the traceback
* **p** is for printing values we want
    For instance: **p return_tensors** to see the value of the variable *return_tensors*
    This way, we can get a look at any variable/value and detect what is wrong
* **q** for quitting the debugger


## Use a debugger in a terminal

1. In a separate chunk, you can execute:
`python -m pdb script_name.py`
* **n** executes the script until the next instruction
* **c** executes the script until the error
* **u** gets you one step upwards in the traceback
* **d** gets you one step downwards in the traceback
* **p** is for printing values we want
    For instance: **p return_tensors** to see the value of the variable *return_tensors*
...
2. Another way is to put a breaking point in the script, such as with the following line of code:
`pdb.set_trace()`
It requires to `import pdb` at the beginning of the script.

It will stop the execution at the breaking point we set.
We can inspect all the variables before the next instruction is executed.

## Debugging the training pipeline

Have a look at the following course that investigates how to debug a training pipeline - that has the particularity to be composed of many processes:
...
* Each AutoModelForXxx class (such as in: model = AutoModelForSequenceClassification) will load a model suitable for a specific task (here, sequence classification)





# Documentation on HuggingFace.co
